  ...
  return (
    <div className="flex flex-col gap-2">
      {safeWeapons.map((weaponEntry, i) => {
        ...
        return (
          <div key={i} className="p-2.5 px-3 flex flex-wrap 2xl:flex-nowrap items-center gap-3 border-l-2 border-primary/50 group hover:border-primary transition-colors">
            
            {/* Left: Name and Traits */}
            <div className="flex flex-col xl:flex-row xl:items-center gap-1 xl:gap-3 flex-grow min-w-[150px]">
       <span className="text-sm text-primary whitespace-nowrap">{item.name}</span>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 shrink-0 w-full sm:w-auto mt-2 2xl:mt-0 ml-auto">
              {isEditing ? (
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => handleRemove(i)} className="text-red-400/50 hover:text-red-400 text-xs p-1 ml-1 cursor-pointer transition-colors shrink-0">
                ...
                <StrikeActionGroup 
                  ...
                  damage={wData?.damage} 
                  traits={traits} 
                  causes={weaponEntry.causes}
                  variant={weaponEntry.delta < 0 ? 'negative' : (weaponEntry.delta > 0 ? 'positive' : null)}
                />
              )}
            </div>
          </div>
      ...
      {isEditing && (
        isAdding ? (
          <select 
            ...
            className="col-span-full border-2 border-primary/40 px-4 py-1.5 flex items-center justify-center text-primary outline-none cursor-pointer mt-2"
            defaultValue=""
          >
          ...
          <button onClick={() => setIsAdding(true)} className="col-span-full border-2 border-primary/40 p-2 flex items-center justify-center text-primary hover:bg-primary/10 hover:border-primary transition-all rounded py-1.5 opacity-60 hover:opacity-100 mt-2">
            + Equip Weapon
          </button>
        )
      )}
