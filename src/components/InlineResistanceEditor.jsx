...
export default function InlineResistanceEditor({ 
    values = [], 
    collectionName, 
    entityId, 
    fieldPath, 
    isEditing, 
    options = [],
    variant = "resistance" // "resistance" or "weakness" (controls colors)
}) {
    ...
    const handleSelect = (traitRaw) => {
        // Add with a default value of 5
        handleChange([...safeValues, { type: traitRaw, value: 5 }]);
        setInputValue('');
    ...
    const isWeakness = variant === "weakness";

    if (!isEditing) {
        if (safeValues.length === 0) return <span className="text-xs text-outline-variant italic">None</span>;
        return (
            <div className="flex gap-1 flex-wrap mb-3">
                {safeValues.map(v => (
                    <StatPill key={v.type} variant={isWeakness ? "negative" : "positive"}>
                        {v.type} {v.value}
                    </StatPill>
                ))}
            </div>
    ...
    const editPillClass = isWeakness
        ? "inline-flex items-center gap-1 bg-red-500/10 border border-red-500/30 px-2 py-0.5 text-[10px] font-bold font-label uppercase tracking-widest text-red-300 shadow-sm"
        : "inline-flex items-center gap-1 bg-surface-container-high border border-primary/30 px-2 py-0.5 text-[10px] font-bold font-label uppercase tracking-widest text-primary shadow-sm";
        ...
        <div ref={wrapperRef} className="relative w-full mb-3">
            ...
            >
                ...
                <input 
                    ...
                    placeholder={safeValues.length === 0 ? `Add ${variant}...` : ""}
