...
export default function InlineResistanceEditor({ 
  ...
  if (!isEditing) {
    ...
    return (
      <div className="flex gap-1 flex-wrap">
        {safeValues.map(v => (
          <StatPill key={v.type} variant={isWeakness ? "negative" : undefined}>
            {v.type} {v.value}
          </StatPill>
        ))}
      </div>
  ...
  ? "inline-flex items-center gap-1 bg-red-500/10 border border-red-500/30 px-2 py-0.5 text-[12px] font-label tracking-widest text-red-300 shadow-sm"
  : "inline-flex items-center gap-1 border border-primary/30 px-2 py-0.5 text-[12px] font-label tracking-widest text-primary shadow-sm";
