...
export default function InlineTraitValueEditor({ 
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <div 
        className={`flex flex-wrap items-center min-h-[32px] gap-1 cursor-text transition-colors w-full relative z-20 border px-2 py-1 rounded ${isFocused ? 'border-primary/50 shadow-sm shadow-primary/20' : 'border-outline-variant/30'}`}
        onClick={() => {
          inputRef.current?.focus();
          setIsFocused(true);
        }}
              ...
              {v.category !== 'none' && (
                <>
                  <input
                    ...
                    className="form-input-number w-8 ml-1 p-0 hide-arrows"
      ...
      {isFocused && (filteredAvailable.length > 0 || inputValue.length > 0) && (
        ...
        className="px-3 py-2 text-xs text-primary hover:bg-primary/20 hover:text-primary cursor-pointer tracking-widest font-label "
              >
                {opt}
              </div>
            ))
