...
export default function DatabaseSeeder() {
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-tertiary p-4 rounded-xl border-2 border-tertiary/50 shadow-lg z-50 text-on-tertiary">
      <h3 className="font-bold text-sm uppercase mb-2">Dev DB Seeder</h3>
      <p className="text-xs mb-3">Push dummy data directly into Firebase?</p>
      <button onClick={seedDatabase} className="bg-black/30 hover:bg-black/50 px-4 py-2 rounded font-bold text-xs uppercase tracking-widest transition-colors w-full">
         {status}
      </button>
    </div>
  );
