    ...
    return (
        <div className="flex flex-wrap gap-1 items-center">
            {safeInv.map((invEntry, idx) => {
                ...
                return (
                    <span key={`${invEntry.itemId}-${idx}`} className="px-2 py-0.5 bg-surface-container-lowest border border-outline-variant/10 text-[10px] text-secondary flex items-center gap-1 group">
                        {displayName} 
                        
                        {isEditing ? (
                            <span className="flex items-center text-primary/70 ml-1 border-l border-outline-variant/20 pl-1 shrink-0">
                                x<InlineEditable type="number" value={invEntry.quantity} collectionName={collectionName} entityId={entityId} isEditing={isEditing} className="w-6 text-center text-[10px] bg-black/50" onSave={(val) => updateInvField(idx, 'quantity', val)} />
                            </span>
                        ) : (
            ...
            {isEditing && availableItems.length > 0 && (
                isAdding ? (
                    <select 
                        ...
                        className="px-2 py-[2px] bg-surface-container-lowest border border-outline-variant/10 text-[10px] outline-none border-dashed border-primary cursor-pointer text-primary"
                        defaultValue=""
                    >
                    ...
                    <button onClick={() => setIsAdding(true)} className="px-2 py-[2px] bg-surface-container-lowest border border-outline-variant/10 text-[10px] text-secondary border-dashed border-primary hover:bg-primary/20 cursor-pointer opacity-70">
                        + Add Item
                    </button>
                )
            )}
