  ...
  return (
    <div className="flex flex-col gap-3">
      {safeSlots.map((slot, i) => (
        <div key={`slot-${i}`} className="flex flex-col md:flex-row md:items-start lg:items-center gap-2 p-2.5 px-3 border-t border-primary bg-[#12111A] group">
     <span className="text-[12px] text-primary shrink-0 flex items-center gap-1 w-auto lg:w-40 pt-1 lg:pt-0">
            Level <InlineEditable type="number" value={slot.level} collectionName="creatures" entityId={entityId} isEditing={isEditing} className="w-8 text-center mx-1 border border-outline-variant/20 rounded" onSave={(val) => handleSlotUpdate(i, 'level', val)} /> 
            <span className="opacity-60 flex items-center normal-case normal-weight">
              (<InlineEditable type="number" value={slot.slots} collectionName="creatures" entityId={entityId} isEditing={isEditing} className="w-8 text-center mx-1 border border-outline-variant/20 rounded" onSave={(val) => handleSlotUpdate(i, 'slots', val)} /> Slots)
            </span>
          </span>
       ...
       pillClass="px-2 py-1 border border-outline-variant/20 text-[12px] tracking-wider text-primary "
          ...
          {isEditing && (
            <button onClick={() => handleRemoveSlot(i)} className="text-red-400 hover:text-red-300 ml-auto shrink-0 opacity-50 hover:opacity-100 cursor-pointer text-xs">
              ×
            </button>
          )}
        </div>
      ...
      {isEditing && (
    <button onClick={handleAddSlot} className="text-[12px] text-primary border border-dashed border-primary/30 rounded py-2 mt-2 w-full hover:bg-primary/10 transition-colors">
