...
export default function InlineReferenceArray({ 
  ...
  pillClass = "px-2 py-0.5 border border-outline-variant/10 text-[12px] text-primary",
  ...
  // Hide options already selected and execute any caller-defined search filtering routines
  const availableOptions = optionsSource
    .filter(opt => !safeValues.includes(opt.id))
    .filter(opt => (searchFilter ? searchFilter(opt) : true))
      ...
      {isEditing && (
        isAdding ? (
          <select 
            ...
            className={`${pillClass} outline-none border-primary cursor-pointer max-w-[200px] text-primary`}
            defaultValue=""
          >
