 ...
 return (
  <main className="page-layout-wrapper">
   <div className="flex justify-between items-end border-b pb-4 shrink-0 border-tertiary/30">
    <div>
     <h1>Creatures</h1>
   <p className="text-off-white font-label text-xs tracking-widest opacity-70">NPCs, Aliens, and Hazards</p>
    </div>
   ...
   <div className="flex flex-col gap-3 min-h-0">
    {filteredCreatures.map((c) => {
     ...
     return (
     <Link to={`/creatures/${c.id}`} key={c.id} className="corner-cut p-4 border-l-2 border-primary relative flex items-center justify-between gap-6 group hover:transition-colors cursor-pointer block shrink-0">
      


      <div className="flex-1 flex flex-wrap items-center gap-4">
    <h4 className="text-xl font-headline text-off-white tracking-tight leading-none group-hover:text-white transition-colors">{c.name}</h4>
       <StatPill variant="custom" size="xs" className={`${rarityColor} border`}>{actualRarity}</StatPill>
       <StatPill size="xs" variant="secondary">{c.type || 'Unknown'}</StatPill>
       {c.traits?.filter(t => !['common', 'uncommon', 'rare', 'unique'].includes(t.toLowerCase())).map((t, i) => {
         const isSize = ['tiny', 'small', 'medium', 'large', 'huge', 'gargantuan'].includes(t.toLowerCase());
         return <StatPill key={i} size="xs" variant={isSize ? "secondary" : "primary"}>{t}</StatPill>;
       })}
      </div>

      <div className="text-center pr-6 hidden sm:flex flex-col items-center justify-center">
    <span className="text-[12px] font-label text-primary tracking-widest leading-none mb-1">Level</span>
        <span className="text-xl font-headline text-primary leading-none" title="Level">{c.level}</span>
      </div>
