...
export default function PlayerDetail() {
  ...
  return (
   <main className="page-layout-wrapper">
     ...
     <p className="text-off-white font-label text-xs tracking-widest opacity-70 mt-1">
           {pc.size} • {pc.build?.heritage?.name || pc.heritage || ''} {pc.build?.ancestry?.name || pc.ancestry || ''} • Played by {pc.playerName}
           <span className="ml-4 text-primary ">Hero Points: {pc.heroPoints}</span>
         </p>
        </div>
      </div>
      <div className="flex gap-4 items-center shrink-0">
        {isEditing ? (
         <button onClick={() => setIsDeleteModalOpen(true)} className="w-8 h-8 rounded border border-accent-yellow/40 text-accent-yellow flex items-center justify-center hover:bg-accent-yellow/10 transition-colors" title="Delete PC">
     ...
     <div className="grid grid-cols-12 gap-6">
      ...
      <div className="col-span-12 lg:col-span-3 flex flex-col gap-6">
        ...
        <div className="border border-outline-variant/10 p-4">
         ...
         <div className="grid grid-cols-2 gap-4">
           <div className="p-4 border border-outline-variant/20 flex flex-col items-center relative">
      <span className="text-[12px] font-label text-primary opacity-60 mb-1">HP</span>
            
            <div className={`text-[1.4rem] font-headline leading-none text-center flex items-baseline justify-center whitespace-nowrap ${pc.hp?.current / pc.hp?.max <= 0.5 ? 'text-accent-yellow' : 'text-primary'}`}>
              {hasActiveEncounter ? (
            ...
            {hasActiveEncounter && isEditing && (
              <div className="absolute top-1 right-1 text-accent-yellow" title="Locked By Active Combat">
      ...
      <span className="text-[12px] font-label text-primary opacity-60 mb-1">AC</span>
         ...
         {(isEditing || pc.immunities?.length > 0 || pc.resistances?.length > 0 || pc.weaknesses?.length > 0) && (
           <div className="border-t pt-4 mt-4 border-tertiary/30">
            {(isEditing || pc.immunities?.length > 0) && (
              <div>
               <span className="text-xs text-primary block mb-1">Immunities:</span>
            ...
            {(isEditing || pc.resistances?.length > 0) && (
              <div className="mt-4">
               <span className="text-xs text-primary block mb-1">Resistances:</span>
            ...
            {(isEditing || pc.weaknesses?.length > 0) && (
              <div className="mt-4">
               <span className="text-xs text-primary block mb-1">Weaknesses:</span>
               <InlineResistanceEditor
        ...
        <span className="text-xs text-primary ">Perception</span>
               <div className="w-12"><InlineEditable value={pc.perception} collectionName="players" entityId={pc.id} fieldPath="perception" isEditing={isEditing} type="number" className="text-lg text-primary border-b-[1px] text-center border-tertiary/30" /></div>
              </React.Fragment>
            ) : (
      ...
      <span className="text-xs text-primary ">Class DC</span>
            <span className="text-sm text-primary">{pc.classDC}</span>
           </div>

           <div className="mt-4 pt-4 border-t border-tertiary/30">
      <span className="text-xs text-primary block mb-2">Senses</span>
      ...
      <div className="col-span-12 lg:col-span-9 flex flex-col gap-6">
         ...
         <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
           {['Land', 'Swim', 'Climb', 'Fly', 'Burrow'].map(speedType => {
            ...
            const getAthleticsScore = () => {
              const val = Object.entries(pc.skills || {}).find(([k,v]) => k.toLowerCase() === 'athletics');
              return val ? val[1] : (pc.attributes?.str || 0);
            };
        ...
        <span className={`text-[12px] font-label opacity-60 mb-1 transition-colors ${isInteractive ? 'text-primary group-hover:text-primary' : 'text-primary'}`}>{speedType}</span>
               <div className={`text-3xl font-headline leading-none text-center whitespace-nowrap flex items-center justify-center transition-colors ${isInteractive ? 'text-primary group-hover:text-white' : 'text-primary'}`}>
                 {isEditing ? (
                  <InlineEditable
                    type="number"
        ...
        {(isEditing || (pc.weapons && pc.weapons.length > 0)) && (
        ...
        {/* Inventory */}
        {(isEditing || (pc.inventory && pc.inventory.length > 0)) && (
          <div className="border border-outline-variant/10 p-6 flex flex-col gap-4">
           <div className="flex justify-between items-center mb-2">
             <h3>Inventory</h3>
             <span className="text-xs text-primary flex items-center">
              <InlineEditable type="number" value={pc.credits || 0} collectionName="players" entityId={pc.id} fieldPath="credits" isEditing={isEditing} className="w-16 text-right mx-1 border border-outline-variant/20 rounded" />
              CR
             </span>
           </div>
           <p className="text-xs text-off-white flex items-center">
             <span className="mr-2">Armor:</span>
             {isEditing ? (
              <span className="opacity-50 text-[12px] italic">Edit armor through inventory</span>
             ) : (
        ...
        {/* Features & Feats */}
        {(isEditing || (pc.feats && Object.values(pc.feats).some(arr => arr && arr.length > 0))) && (
          <div className="border border-outline-variant/10 p-6 flex flex-col gap-4">
           <h3>Features & Feats</h3>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {['ancestry', 'class', 'general', 'skill'].map(type => (
              <div key={type} className="border-t pt-3 border-tertiary/30">
         <span className="text-[12px] text-primary tracking-widest block mb-2">{type} Feats</span>
                <div className="flex flex-wrap gap-2">
                 {pc.feats && pc.feats[type] && pc.feats[type].length > 0 ? pc.feats[type].map(feat => {
                   const fName = typeof feat === 'object' ? (feat.name || 'Unnamed Feat') : feat;
                   const uKey = typeof feat === 'object' ? (feat._id || fName || Math.random()) : feat;
                   return <span key={uKey} className="px-3 py-1.5 border border-outline-variant/30 text-xs text-primary">{fName}</span>
