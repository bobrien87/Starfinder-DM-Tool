...
export default function ItemListing() {
 ...
 return (
  <main className="page-layout-wrapper">
   <div className="flex justify-between items-end border-b pb-4 shrink-0 border-tertiary/30">
    <div>
     <h1>Items</h1>
   <p className="text-off-white font-label text-xs tracking-widest opacity-70">Weapons, Armor, Gear, & Upgrades</p>
    </div>
   ...
   <div className="flex flex-wrap gap-4 p-4 border border-outline-variant/10 shrink-0 relative z-20">
    ...
    <label className="text-[12px] text-primary tracking-widest">Rarity</label>
     ...
     <div className="flex flex-col gap-1 w-full sm:w-auto">
    <label className="text-[12px] text-primary tracking-widest">Type</label>
       <CustomSelect 
         value={filterType}
    ...
    <label className="text-[12px] text-primary tracking-widest">Filter by Traits</label>
       <CustomMultiSelect 
         value={filterTraits} 
     ...
     <div className="flex flex-col gap-1 w-full sm:w-auto">
    <label className="text-[12px] text-primary tracking-widest">Price Range</label>
   ...
   <div className="flex flex-col gap-3 min-h-0">
    {filteredItems.map((i) => {
     ...
     return (
     <Link to={`/items/${i.id}`} key={i.id} className="corner-cut p-4 border-l-2 border-primary relative flex items-center justify-between gap-6 group hover:transition-colors cursor-pointer block shrink-0">
      
      
      <div className="flex-1 flex flex-col gap-1">
    <h4 className="text-xl font-headline text-off-white tracking-tight leading-none group-hover:text-white transition-colors">{i.name}</h4>
        <div className="flex flex-wrap gap-1.5 items-center">
          <StatPill variant="custom" size="xs" className={`${rarityColor} border`}>{actualRarity}</StatPill>
          <StatPill size="xs" variant="secondary">{i.type}</StatPill>
          <div className="flex gap-1 flex-wrap mt-2 overflow-hidden max-h-[18px]">
             {filteredTargetTraits.slice(0, 5).map(trait => {
               const isSize = ['tiny', 'small', 'medium', 'large', 'huge', 'gargantuan'].includes(trait.toLowerCase());
               return <StatPill key={trait} size="xs" variant={isSize ? "secondary" : "primary"}>{trait}</StatPill>;
             })}
      ...
      <div className="text-center pr-6 hidden sm:flex items-center gap-6">
        <div className="flex flex-col items-center justify-center">
     <span className="text-[12px] font-label text-primary tracking-widest leading-none mb-1">Price</span>
          <span className="text-xl font-headline text-primary leading-none">{i.price || 0}</span>
        </div>
        <div className="flex flex-col items-center justify-center border-l border-outline-variant/20 pl-6">
     <span className="text-[12px] font-label text-primary tracking-widest leading-none mb-1">Level</span>
          <span className="text-xl font-headline text-primary leading-none" title="Level">{i.level}</span>
        </div>
      </div>
